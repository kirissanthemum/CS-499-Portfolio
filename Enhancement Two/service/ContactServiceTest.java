...
class ContactServiceTest {
	...
	//calls AddContact method, validates first name, last name, phone, and address, validate list isn't empy
	
	@DisplayName("Test addContact")
	@Test
	void testAddContact() { //typo fixed
		...
		assertEquals("0", ContactService.contactList.get(0).getContactID()); //updated
	...
	//ENHANCEMENT BELOW
	//Add update failure tests to ensure that invalid inputs are rejected
	//First Name update failure test
	@DisplayName("Test updateFirstName fail cases")
	@Test
	void testUpdateFirstNameFailures() {
	    ContactService test = new ContactService();
	    test.addContact("Kirissa", "Byington", "1234567890", "123 South St");
	    ...
	    assertThrows(IllegalArgumentException.class, () -> {
	        test.updateFirstName("0", "");
	    });

	    // Too long first name
	    assertThrows(IllegalArgumentException.class, () -> {
	        test.updateFirstName("0", "Name is wayyyyyyyy tooooo long");
	    });
	}
	
	//Phone number update failure test
	@DisplayName("Test updatePhone fail cases")
	@Test
	void testUpdatePhoneFailures() {
	    ContactService test = new ContactService();
	    test.addContact("Kirissa", "Byington", "1234567890", "123 South St");
	    ...
	    assertThrows(IllegalArgumentException.class, () -> {
	        test.updatePhone("0", "123"); // way too short
	    });

	    assertThrows(IllegalArgumentException.class, () -> {
	        test.updatePhone("0", "123456789012345"); // way too long
	    });
	}
